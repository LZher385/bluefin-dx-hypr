...
# warning, and hypridle never starts (it refuses to run without a config).
# Ship a systemd --user oneshot that copies each missing file into $HOME
# on login. `cp -n` leaves any existing user-edited file alone.
...
[Unit]
Description=Seed default Hyprland configs from /etc/skel

[Service]
Type=oneshot
# cp -n leaves any user-edited file alone; the loop covers every skel file
# under ~/.config/hypr (hyprland.lua, hypridle.conf, hyprlock.conf, ...).
ExecStart=/bin/sh -c '\
    mkdir -p "$HOME/.config/hypr" && \
    for f in /etc/skel/.config/hypr/*; do \
        [ -f "$f" ] && cp -n "$f" "$HOME/.config/hypr/"; \
    done'
