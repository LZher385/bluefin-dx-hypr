...
# image never get the default hyprland.lua, so Hyprland autogenerates a
# bare one and emits a "you're on the autogenerated config" warning.
# Ship a systemd --user oneshot that copies the default into $HOME on
# login when it's missing. cp -n + ConditionPathExists guard against
# ever clobbering a user edit.
...
ConditionPathExists=!%h/.config/hypr/hyprland.lua
ConditionPathExists=/etc/skel/.config/hypr/hyprland.lua

[Service]
Type=oneshot
ExecStart=/bin/sh -c 'mkdir -p "$HOME/.config/hypr" && cp -n /etc/skel/.config/hypr/hyprland.lua "$HOME/.config/hypr/hyprland.lua"'
